...
supabase.auth.onAuthStateChange(async (event, session) => {
  if (session) {
    ...
    currentUser.memberId = profile ? profile.member_id : user.id.slice(0, 8);
    currentUser.name = profile ? profile.name : "Customer";
    currentUser.contact = profile ? profile.contact : "";
    currentUser.mudraGold = profile ? profile.mudra_gold : 0;
    currentUser.mudraSilver = profile ? profile.mudra_silver : 0;
...
async function handleSignup(event) {
  ...
  if (error) {
    ...
    alert("OTP request sent successfully to " + contact);
  }
}
...
// Single Consolidated Orders Modal Display Function
function showMyOrdersModal() {
  const container = document.getElementById('ordersListContainer');
  if (!container) return;

  if (currentUser.orders.length === 0) {
    container.innerHTML = "<p style='color:#ccc;'>No orders requested yet.</p>";
  } else {
    container.innerHTML = currentUser.orders.map(o => `
      <div style="background:#222; padding:12px; margin:8px 0; border-radius:6px; display:flex; justify-content:space-between; align-items:center; border:1px solid #333;">
        ...
        ${o.status !== 'Cancelled' ? `
          <button class="btn-gold-action" style="background:#ff4d4d; color:#fff; padding:6px 12px; font-size:0.8rem; margin-left:10px; border:none; cursor:pointer;" onclick="cancelOrder('${o.orderId}')">Cancel Order</button>
